package org.swiftotter
    ...
    def pushArtifactToDeployServer(String nodeName = 'deploy', String sshUser, String sshHost, String sshPort, String sshKey, String sshPath, String buildFile, String buildNumber) {
        ...
        this.executeInNode(nodeName, sshKey) { SSH_KEY ->
            sh 'ssh -i ${SSH_KEY} -o StrictHostKeyChecking=no -c aes256-ctr -p ' + sshPort + ' ' + userHost + ' -vvv \'mkdir -p ' + sshPath + '/releases\''
        }
        
        this.executeInNode(nodeName, sshKey) { SSH_KEY ->
            sh 'scp -i ${SSH_KEY} -P ' + sshPort + ' -c aes256-ctr -v -o StrictHostKeyChecking=no -o \'CompressionLevel 9\' -o \'IPQoS throughput\' ' + buildFile + ' ' + userHost + ':' + sshPath + '/releases/' + buildFile
        }
    }
    ...
    def deployArtifactOnServer(String nodeName = 'deploy', String sshUser, String sshHost, String sshPort, String sshKey, String sshPath, String httpPath, String buildFile, String buildNumber, magentoVersion) {
        ...
        this.executeInNode(nodeName, sshKey) { SSH_KEY ->
            sh 'ssh -i ${SSH_KEY} -c aes256-ctr -o StrictHostKeyChecking=no -p ' + sshPort + ' ' + userHost + ' -vvv << EOF\n' +
